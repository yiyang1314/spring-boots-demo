...
//		List<User> plainUsers2 = userMapper
//				.selectList(new QueryWrapper<User>().inSql("role_id", "select id from role where id = 2"));
//		List<User> lambdaUsers2 = userMapper.selectList(
//				new QueryWrapper<User>().lambda().inSql(User::getRoleId, "select id from role where id = 2"));
...
//				.nested(i -> i.eq("role_id", 2L).or().eq("role_id", 3L)).and(i -> i.ge("age", 20)));
//		List<User> lambdaUsers3 = userMapper.selectList(new QueryWrapper<User>().lambda()
//				.nested(i -> i.eq(User::getRoleId, 2L).or().eq(User::getRoleId, 3L)).and(i -> i.ge(User::getAge, 20)));
